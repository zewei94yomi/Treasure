...
class Game {
  ...
  hordeUpdate(dt) {
    ...
    // —— 刷怪泵：随时间加速加量 ——
    H.spawnT -= dt;
    const interval = Math.max(0.28, 2.2 - t * 0.004);   // 中期收紧、后期 0.28s
    if (H.spawnT <= 0) {
      H.spawnT = interval;
  ...
  hordeGems(dt) {
    ...
    for (const g of H.gems) {
      ...
      if (bd < 22) {
        ...
        this.hordeAddXp(Math.round(g.v * (H.mods.gemMul || 1)));
      }
    }
  ...
  hordeAddXp(v) {
    ...
    while (H.xp >= H.xpNeed) {
      H.xp -= H.xpNeed;
      H.level++;
      H.xpNeed = Math.round((8 + H.level * 4.5) * (this.mode === 2 ? 1.35 : 1));
      H.freeChoices++;
      Sfx.extract();
  ...
  updatePlayer(p, dt) {
    ...
    // 碎金磁吸拾取（与经验宝石同款磁场）
    {
      const magR = this.horde ? 95 * this.hordeState.mods.magnet : 80;
